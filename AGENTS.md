...
- [`docs/STRATEGY.md`](docs/STRATEGY.md) — 北极星、渗透 vs Agent、阶段 VS-1~4
- [`docs/VSCODE_DISTRIBUTION.md`](docs/VSCODE_DISTRIBUTION.md) — 技术契约与设置键
- [`ROADMAP.md`](ROADMAP.md) — 里程碑勾选
...
**VS-1（当前）**：共享 `resolve_kernel_action`（policy-first）；VS Code 经 `oclive-cli kernel ensure --plan-only` attach/spawn；`scene_id=vscode`；共 `app.db` + `OCLIVE_ROLES_DIR`。默认 `skip_agent`；渗透优先宿主实现，见战略 §3。
